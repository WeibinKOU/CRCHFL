...
def game_loop(args):
    ...
    try:
        ...
        camera_r_spawn_point = carla.Transform(cam_r_location,cam_r_rotation)

        cam_l_location = carla.Location(1.0,-1.2,0.5)
        ...
        thread_obj = threading.Thread(target=get_queue, args=(sensor_list, sensor_queue))
        thread_obj.start()
        time.sleep(0.6)
        ...
        def predict(sensor_list, control_queue):
            ...
            rgb_f = torch.rand([1, 3, IMG_HEIGHT, IMG_WIDTH], dtype=torch.float32).cuda()
            rgb_l = torch.rand([1, 3, IMG_HEIGHT, IMG_WIDTH], dtype=torch.float32).cuda()
            rgb_r = torch.rand([1, 3, IMG_HEIGHT, IMG_WIDTH], dtype=torch.float32).cuda()
            rgb_b = torch.rand([1, 3, IMG_HEIGHT, IMG_WIDTH], dtype=torch.float32).cuda()

            _, _ = action_model(rgb_f, rgb_l, rgb_r, rgb_b)
